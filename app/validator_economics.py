...
class ValidatorEconomics:
    ...
    def can_register_validator(self, address: str, balance: int, current_block_height: Optional[int] = None) -> Tuple[bool, str]:
        ...
        if balance < required_deposit:
            return (False, 
                   f"Insufficient balance: need {required_deposit / config.PALS_PER_TMPL} TMPL, "
                   f"have {balance / config.PALS_PER_TMPL} TMPL")
    ...
    def process_validator_deposit(self, address: str, amount: int, current_block_height: Optional[int] = None) -> Tuple[bool, str]:
        ...
        print(f"💰 Validator deposit recorded: {address} deposited {amount / config.PALS_PER_TMPL} TMPL")
        return (True, f"Deposit recorded: {amount / config.PALS_PER_TMPL} TMPL")
    ...
    def slash_validator(self, address: str, reason: str, percentage: int) -> Tuple[bool, int]:
        ...
        print(f"⚔️  SLASHING: {address} slashed {slash_amount / config.PALS_PER_TMPL} TMPL for {reason}")
        print(f"   Remaining deposit: {self.deposits[address] / config.PALS_PER_TMPL} TMPL")
        print(f"   Pending redistribution: {self.pending_redistribution / config.PALS_PER_TMPL} TMPL (will distribute to honest validators in next block)")
    ...
    def process_withdrawal(self, address: str, current_height: int) -> Tuple[bool, int, str]:
        ...
        print(f"💸 Withdrawal processed: {address} withdrew {withdrawal_amount / config.PALS_PER_TMPL} TMPL")
        return (True, withdrawal_amount, f"Withdrew {withdrawal_amount / config.PALS_PER_TMPL} TMPL")
    ...
    def get_redistribution_rewards(self, active_validators: List[str]) -> Dict[str, int]:
        ...
        print(f"💰 REDISTRIBUTION: {self.pending_redistribution / config.PALS_PER_TMPL} TMPL slashed coins")
        print(f"   Distributed to {len(honest_validators)} honest validators (excluding slashed validators)")
        print(f"   Each validator receives: {reward_per_validator / config.PALS_PER_TMPL} TMPL")
    ...
    def enforce_required_deposit(self, address: str, balance: int, current_block_height: int) -> Tuple[bool, int, str]:
        ...
        if balance >= required:
            ...
            self.mark_active(address)
            print(f"🔒 AUTO-LOCKED: {address} deposited {required / config.PALS_PER_TMPL} TMPL (grace period ended)")
            return (True, required, f"Auto-locked {required / config.PALS_PER_TMPL} TMPL deposit")
        else:
            # Insufficient balance - mark inactive
            self.mark_inactive(address, f"insufficient balance: need {required / config.PALS_PER_TMPL} TMPL, have {balance / config.PALS_PER_TMPL} TMPL")
            return (False, 0,
                   f"Validator {address} marked INACTIVE: "
                   f"need {required / config.PALS_PER_TMPL} TMPL, have {balance / config.PALS_PER_TMPL} TMPL")
    ...
    def schedule_deposit(self, address: str, current_block_height: int) -> Tuple[bool, str]:
        ...
        print(f"📅 DEPOSIT SCHEDULED: {address} will auto-lock 100 TMPL at block {self.TRANSITION_BLOCK}")
    ...
    def process_transition(self, registered_validators: List[str], get_balance_func: Callable[[str], int]) -> Dict[str, Tuple[bool, int, str]]:
        ...
        for address in registered_validators:
            ...
            if should_lock and balance >= self.VALIDATOR_DEPOSIT_PALS:
                ...
                print(f"  ✅ {address}: ACTIVE ({reason}, {balance / config.PALS_PER_TMPL:.4f} TMPL → {self.VALIDATOR_DEPOSIT_PALS / config.PALS_PER_TMPL:.0f} TMPL locked)")
                ...
                reason = "auto-lock disabled" if not auto_lock else f"insufficient balance ({balance / config.PALS_PER_TMPL:.4f} TMPL < 100 TMPL)"
