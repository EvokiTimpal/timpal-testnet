# Apply testnet cross-platform storage patch FIRST (LMDB on macOS, LevelDB on Linux)
# This must happen BEFORE any blockchain module imports
import sys
import os
# Check if we're running from app/ or root directory
if 'app' not in sys.path[0]:
    sys.path.insert(0, os.path.join(os.path.dirname(__file__)))

from testnet_patch import apply_testnet_patch
apply_testnet_patch()
...
def get_ledger() -> Ledger:
    ...
    if _ledger_cache is None or (current_time - _ledger_cache_time) > CACHE_TTL:
        _ledger_cache = Ledger(data_dir="testnet_data_node_9000/ledger", use_production_storage=False)
...
async def get_block_by_height(request: Request, height: int):
    ...
    <body>
        ...
        <div class="card">
            ...
            <table class="table">
                <tbody>
                    ...
                    <tr>
                        <td><strong>New TMPL Minted</strong></td>
        ...
        <div class="card">
            <h2>💸 TMPL Transfers ({len(transactions)})</h2>
            {tx_section}
        </div>
...
async def get_transactions(request: Request, page: int = 1, tx_filter: Optional[str] = None, hide_legacy: str = "true"):
    ...
    <body>
        ...
        <div class="card">
            ...
            <p style="color: var(--text-secondary);">
                {f"Showing {len(result['transactions'])} of {result['total_count']:,} transfers (Page {page} of {result['total_pages']})" if result['total_count'] > 0 else 'No transfers found yet - be the first to send TMPL!'}
            </p>
        </div>
        ...
        <div class="card">
            ...
            <div class="stats">
                ...
                <div class="stat-card">
                    ...
                    <div class="stat-trend">TMPL per transfer</div>
                </div>
            </div>
        </div>
        ...
        <div class="card">
            <table class="table">
                ...
                <tbody>
                    {tx_rows if tx_rows else f'<tr><td colspan="8" style="text-align: center; padding: 40px; color: var(--text-secondary);">{"No money transfers yet - blockchain is waiting for users to send TMPL" if tx_filter == "transfer" else "No transactions found"}</td></tr>'}
                </tbody>
            </table>
        </div>
    ...
    <body>
        ...
        <div class="container">
            ...
            <div class="info-box">
                ...
                <strong>💰 Fixed Fee:</strong> 0.0005 TMPL per transaction<br>
            ...
            new Chart(rewardCtx, {
                ...
                options: {
                    ...
                    plugins: {
                        ...
                        title: {
                            display: true,
                            text: 'Total Supply Distribution (TMPL)'
                        },
                        tooltip: {
                            callbacks: {
                                label: function(context) {
                                    ...
                                    label += context.parsed.toLocaleString() + ' TMPL';
                                    return label;
                                }
                            }
                        }
            ...
            new Chart(supplyCtx, {{
                ...
                options: {{
                    ...
                    plugins: {{
                        ...
                        title: {{
                            display: true,
                            text: 'TMPL Supply Growth Over Time'
                        }}
                    }},
                    scales: {{
                        x: {{ title: {{ display: true, text: 'Block Height' }} }},
                        y: {{ 
                            title: {{ display: true, text: 'Supply (TMPL)' }},
                            beginAtZero: true
                        }}
                    }}
                }}
